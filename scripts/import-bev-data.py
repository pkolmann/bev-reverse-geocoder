        ...
        with open(args.file) as f:
            ...
            for line in csv.reader(f, quotechar='"', delimiter=";", quoting=csv.QUOTE_MINIMAL):
                ...
                statement = "INSERT INTO " + \
                            args.table + \
                            "(municipality, locality, postcode, street, house_number, subaddress, house_name, address_type, house_attribute, point)" \
                            "VALUES (%s, %s, %s, %s, %s, %s, %s, 'unknown', %s, ST_SetSRID(ST_MakePoint(%s, %s),4326))"
                ...
                if len(line) == 20 and (is_float(line[8]) or is_float(line[11])) and (is_float(line[9]) or is_float(line[12])):
                    try:
                        ...
                        if is_float(line[8]):
                            cursor.execute(statement, (
                                line[0], line[1], int(line[2]), line[3], line[6], line[13], line[7], line[19], line[8], line[9],)
                           )
                        else:
                            cursor.execute(statement, (
                                line[0], line[1], int(line[2]), line[3], line[6], line[13], line[7], line[19], line[11], line[12],)
                           )
